...
class ApiClient:
    ...
    def proceed_with_plots(self) -> None:
        ...
        with open(self.progress_file, 'w') as f:
            f.write(f'All plots: {len(self.plots)}\n')
    ...
    def get_plots_for_order_id(self, order_id: str, rewrite: bool = False) -> List[Plot]:
        ...
        for plot in response.json().get('plots', []):
            plots.append(Plot(plot_id=plot.get('id'),
                              state=PlotState(plot.get('state')),
                              progress=plot.get('progress'),
                              url=plot.get('url')))
        log.info(f'Found {len(plots)} plots for the order ID={order_id}.')
        if rewrite:
            self.plots = plots
        return plots

    def get_plots_for_order(self, order: Order, rewrite: bool = False) -> List[Plot]:
        """Get all the plots for the given order.

        :param rewrite: whether to store plots it got to self.plots or not.
        """
        return self.get_plots_for_order_id(order.order_id, rewrite)
